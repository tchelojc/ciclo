...
# Example for theme loading
    ...
    def _calculate_cosmic_data(self, year: int, month=None, day=None) -> dict:
        ...
        try:
            ...
            if hasattr(self, 'astronomy') and self.astronomy:
                return {
                    ...
                    'galactic_progress': self.astronomy.galactic_year_progress(date),
                    'earth_angle': self.astronomy.earth_heliocentric_position(date),
        ...
        return {
            'baktun': (year + 3114) / 144000,
            'venus_phase': ((year * 584) % 360),
            'galactic_progress': ((year + 3114) % 26000) / 26000 * 100,
            'earth_angle': (year * 365) % 360,
            'is_aligned': False
        }
    ...
    def get_planetary_positions(self, date):
        """Obtém posições planetárias usando a classe MayanAstronomy"""
        try:
            if hasattr(self, 'astronomy') and self.astronomy:
