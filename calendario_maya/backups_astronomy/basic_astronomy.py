...
class BasicAstronomy:
    ...
    def get_planetary_positions(self, date):
        """Implementação básica usando JD"""
        jd = self.date_to_jd(date)
        return {
            'earth': {
                'position': self._calculate_angle(jd, self.earth_period),
                'jd': jd,
                'cycle': 'helio_continuous'
            },
            'venus': {
                'position': self._calculate_angle(jd, self.venus_period),
                'jd': jd,
                'cycle': 'synodic_continuous'
            }
        }
