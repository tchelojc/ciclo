...
venus_pos = mayan_astro.get_planetary_positions(date_func.today())['venus']
...
class StarMap(QGraphicsView):
    def __init__(self, hemisphere="Norte", parent=None):
        ...
        # Inicializa com posições padrão
        self.venus_pos = {'position': 0, 'ra': 0, 'dec': 0}
        try:
            from datetime import date as date_func
            mayan_astro = MayanAstronomy()
            positions = mayan_astro.get_planetary_positions(date_func.today())
            if positions and 'venus' in positions:
                self.venus_pos = positions['venus']
        except Exception as e:
            print(f"Erro ao carregar posições planetárias: {e}")
